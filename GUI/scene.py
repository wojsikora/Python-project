...
class ShopScene:
    ...
    def create_items_to_shop(self):
        ...
        for i in range(len(self.shopkeeper.goods)):
            self.item_views_shop.append(
                ItemView(self.menuRect.x + 10 + (i % 5) * 50, self.menuRect.y + 10 + (i // 5) * 50, 40, 40,
                         self.shopkeeper.goods[i]))

            self.price_views.append(TextView(
                pygame.rect.Rect(self.item_views_shop[i].x, self.item_views_shop[i].y + self.item_views_shop[i].h + 5,
                                 self.item_views_shop[i].w, 50), self.game.graphics.font, (0, 0, 0), (100, 100, 100),
                str(self.shopkeeper.goods[i].price)))
        j = 0
        for i in range(len(self.game.map.player.inventory)):
            if not self.game.map.player.is_item_used(self.game.map.player.inventory[i]):
                self.item_views_inventory.append(
                    ItemView(self.inventoryRect.x + (j % 5) * 50 + 10, self.inventoryRect.y + 350 + (j // 5) * 50, 40,
                             40, self.game.map.player.inventory[i]))
                j += 1
    ...
    def find_clicked_item_view(self, x, y, list):
        ...
        for item_view in list:
            if item_view.x <= x <= item_view.x + item_view.w and item_view.y <= y <= item_view.y + item_view.h:
                return i
            i += 1
        # nie ma itemu
        return -1
