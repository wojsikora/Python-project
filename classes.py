    ...
    def update(self,elapsed):
        ...
        if self.pressed_axe or self.pressed_weapon:
            ...
            if offset_x!=0 or offset_y!=0:
                #wspólrzedne gracza
                #if self.field.x+offset_x >=0 and self.field.x+offset_x<self.game.map.width and self.field.y+offset_y >=0 and self.field.y + offset_y <self.game.map.width:
            ...
            p_x=self.x+self.velocity_x*elapsed/1000
            p_y=self.y+self.velocity_y*elapsed/1000
            ...
            for obj in self.game.map.fields[int(p_x)][int(p_y)].objects:
                if self is obj:
                    continue
