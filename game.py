...
import sys
from level_information import Level_Information
...
class Game:
    ...
    def next_level(self):
        ...
        self.map.leader = Leader(self)
        field = self.map.get_no_bounadry_field()
        self.map.set_position(self.map.leader, field.x, field.y)
        self.map.leaders.append(self.map.leader)
        if self.level%3==1:
